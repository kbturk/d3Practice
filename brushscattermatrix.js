...
function safeExtent(xs, fallback, projection) {
    ...
    function brush(cell, circle, svg) {
        ...
        function brushed({ selection }, [i, j]) {
            ...
            if (selection) {
                ...
                circle.classed("hidden", d => x0 > x[i](d[columns[i]])
                    || x1 < x[i](d[columns[i]])
                    || y0 > y[j](d[columns[j]])
                    || y1 < y[j](d[columns[j]]));
                selected = data.filter(d => x0 < x[i](d[columns[i]])
                    && x1 > x[i](d[columns[i]])
                    && y0 < y[j](d[columns[j]])
                    && y1 > y[j](d[columns[j]]));
            }
...
//# sourceMappingURL=brushscattermatrix.js.map